...
def get_access_token():
    ...
    token_string = open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'api_token.txt'), 'w+')
...
def read_access_token():
    access_token = ""
    file = open(os.path.dirname(os.path.abspath(__file__)) + '/api_token.txt', 'r')
    for line in file:
...
def check_token():
    if os.path.isfile(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'api_token.txt')):
        ...
        print("\nYou first need to generate your token in order to being able to use the API service!\n")
        get_access_token()
        print("\nToken generated successfully!")
